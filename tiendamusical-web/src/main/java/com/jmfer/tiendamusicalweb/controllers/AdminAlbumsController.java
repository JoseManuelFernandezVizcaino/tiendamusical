...
public class AdminAlbumsController {
	...
	/**
	 * Albums a mostrase en el datatable
	 */
	...
	/**
	 * Objeto que se utiliza para almacenar el archivo de la imagem del album a cargar de forma temporal
	 */
	...
	/**
	 * Directorio donde se almacena las imagenes de albums
	 */
	...
	@PostConstruct
	public void init() {
		LOGGER.info("Incializando la pantalla de albums del administrador...");
		this.consultar();
	...
	/**
	 * Metodo que permite inicializar la informacion de los componente de carga en los desplegables
	 */
	...
	/**
	 * Metodo que permite inicializar o limpiar los compoenentes u objetos utilizados en el formulario
	 */
    ...
    /**
     * Metodoq que permite guardar el album
     */
    public void guardar() {
    	try {
			CommonUtils.guardarImagen(this.absolutePath, this.uploadedFile.getFileName(), this.inputStream);
		} catch (IOException e) {
			CommonUtils.mostrarMensaje(FacesMessage.SEVERITY_ERROR, "ERROR!", "Hubo un problema al guardar la imagen en el directorio indicado, por favor contactar con soporte");
			e.printStackTrace();
		}
    	...
    	if (albumGuardado.getIdAlbum() != null) {
			CommonUtils.mostrarMensaje(FacesMessage.SEVERITY_INFO, "OK!", "Album " + this.album.getNombre() + "guardado exitosamente");
    ...
    /**
     * Metodo que permite precargar el album selecionado para actualizar
