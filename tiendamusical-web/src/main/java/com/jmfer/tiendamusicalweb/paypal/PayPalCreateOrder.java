...
public class PayPalCreateOrder extends PayPalClient {
	...
	private OrderRequest generarCuerpoOrden(SessionBean sessionBean) {
		...
		ApplicationContext applicationContext = new ApplicationContext().brandName("jmfer INC.")
		...
		shippingDetail.addressPortable(new AddressPortable().addressLine1("C/Hermanos Machado")
															.addressLine2("20A")
															.adminArea1("ESPAÑA")
															.adminArea2("Sanlucar la Mayor")
															.postalCode("41800")
															.countryCode("ES"));
		...
		double roundedTotal = (double)Math.round(sessionBean.getTotalCompra() * 100d) / 100d;
		...
		double totalCompraConImpuesto = roundedTotal + (impuestoSumaTotalProductos + handling + (envio - descuentoEnvio));
		...
		purchaseUnitRequest.amountWithBreakdown(new AmountWithBreakdown()
				...
				.amountBreakdown(new AmountBreakdown()
						.itemTotal(new Money().currencyCode("EUR").value(String.valueOf(roundedTotal)))
